...
This standard connects the grounded exercise imagery already created for LetMeFly with the newer cinematic black / steel / purple application theme. It is presentation-only. Exercise mappings, program prescriptions, workout history, readiness, progression, substitutions, and athlete data remain authoritative and unchanged.

## Core principle

LetMeFly intentionally uses two related visual layers:

1. **Identity / world art** — Raizen, Fenrir, mountains, storms, crowns, runes, purple lightning, milestones, banners, and motivational imagery.
...
- purple appears mainly at the card edge/selection treatment, not as a heavy image tint
...
- subtle purple edge glow/border connects the image to the LetMeFly palette
- hover/focus/selection may increase purple edge emphasis
...
- use only light purple perimeter treatment
...
- use the same black/steel frame and subtle purple edge treatment
...
Purple is an integration accent, not an image recolor.

Use purple for:

- active/selected borders
- subtle edge glow
- tiny perimeter gradients
- focus states
- card accents

Do not use heavy full-image purple overlays that make the exercise harder to read.
...
- Exercises thumbnails feel integrated with black/purple LetMeFly styling without obscuring the movement
...
Implementation reference: `exercise-image-theme-v1` plus the approved Train blended-card treatment.
