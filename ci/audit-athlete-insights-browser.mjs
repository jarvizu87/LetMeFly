  ...
  return page.evaluate(async () => {
    ...
    await new Promise((resolve, reject) => {
      ...
      for (const [i, days, load] of [[1, 1, 100], [2, 3, 200], [3, 35, 50], [4, 40, 50]]) {
        ...
        tx.objectStore('workoutSessions').put({ id: session, athlete_id: athlete.id, completed_at: date, started_at: date, status: 'completed', program_key: 'crownforge', workout_name: `Evidence ${i}` })
        tx.objectStore('workoutExercises').put({ id: exercise, athlete_id: athlete.id, workout_session_id: session, exercise_key: 'bench-press', exercise_name_snapshot: 'Bench Press' })
        tx.objectStore('workoutSets').put({ id: `set-${i}`, athlete_id: athlete.id, workout_session_id: session, workout_exercise_id: exercise, completed: true, completed_at: date, load_value: load, load_unit: 'kg', reps: 5, rpe: i === 1 ? 8 : 6 })
      }
      ...
      tx.objectStore('workoutExercises').put({ id: 'carry', athlete_id: athlete.id, workout_session_id: 'insight-1', exercise_key: 'farmer-carry', exercise_name_snapshot: 'Farmer Carry' })
      tx.objectStore('workoutSets').put({ id: 'carry-set', athlete_id: athlete.id, workout_session_id: 'insight-1', workout_exercise_id: 'carry', completed: true, completed_at: date, load_value: 50, load_unit: 'kg', reps: 20, performance_data: { actualMetricKind: 'distance', actualMetricValue: 20, actualMetricUnit: 'm' } })
...
try {
  for (const width of [412, 1440]) {
    ...
    report.checks.push({ viewport: width, result: 'PASS', coverage: ['actual volume', 'period comparison', 'metric exclusion', 'exercise filter', 'chart', 'route cleanup', 'Coach source context', 'athlete isolation', 'reload', 'source/outbox immutability', ...(process.env.CHECK_PROGRAM_ORDER === '1' ? ['program order'] : [])] })
    await context.close()
  }
