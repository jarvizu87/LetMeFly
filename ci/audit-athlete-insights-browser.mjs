...
try {
  for (const width of [412, 1440]) {
    ...
    report.checks.push({ viewport: width, result: 'PASS', coverage: ['actual volume', 'period comparison', 'metric exclusion', 'exercise filter', 'chart', 'route cleanup', 'Coach source context', 'athlete isolation', 'reload', 'source/outbox immutability', 'empty profile', 'native profile save and Coach reload', 'literal profile text', ...(process.env.CHECK_PROGRAM_ORDER === '1' ? ['program order'] : [])] })
    await context.close()
  }
