#!/usr/bin/env node
// Supplemental #57-59 proof. Real candidate services and native IndexedDB only.
...
if (provenance.productionSha !== 'b6afd3d4125a3f89519a5032867975f99b1bae8b' || !provenance.candidateOnly) throw new Error('Exact unmerged candidate kit required')
for (const [file, hash] of Object.entries(provenance.sourceHashes)) {
  if (crypto.createHash('sha256').update(fs.readFileSync(path.join(app, file))).digest('hex') !== hash) throw new Error(`Source mismatch: ${file}`)
}
...
const report = { result: 'RUNNING', testedApplicationCommit: provenance.productionSha, baselineProductionCommit: provenance.baselineProductionSha, candidateOnly: true, passes: [], failures: [], externalRequests: [] }
...
try {
  for (let iteration = 1; iteration <= 3; iteration++) {
    ...
    await run(`cross-tab concurrent advancement ${iteration}`, async (page, context, fixture) => {
      ...
      const other = await context.newPage(); await visit(other)
      const advance = p => p.evaluate(f => window.__LMF_LIFECYCLE_Q__.progression.advanceProgramAfterWorkout(f.athleteId, 'crownforge', 1, 1), fixture)
      // Invoke the real service independently in each page, sharing only IndexedDB.
