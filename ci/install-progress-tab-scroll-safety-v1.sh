...
# the tabpanel. Preserve a mounted-session height floor equal to the largest panel
# height encountered so switching from a taller panel to a shorter panel cannot
# shrink the document underneath the user's current scroll position. The floor
# naturally resets when the Progress dashboard unmounts. This is presentation-only
# and never writes athlete or program data.
...
grep -Fq 'window.setTimeout(restoreScroll,260)' "$PROGRESS_JS"
grep -Fq 'window.setTimeout(restoreScroll,360)' "$PROGRESS_JS"
! grep -Fq 'previousMinHeight' "$PROGRESS_JS"
! grep -Fq 'panel.style.minHeight=previousMinHeight' "$PROGRESS_JS"
