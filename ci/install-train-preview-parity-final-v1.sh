...
text=re.sub(r'</head>','  <link rel="stylesheet" href="/ui/train-preview-parity-final-v1.css?v=3">\n</head>',text,count=1,flags=re.I)
...
existing=re.findall(r"['\"]([^'\"]+)['\"]",m.group(1)); required=['/ui/train-preview-parity-final-v1.css','/ui/train-preview-parity-final-v1.css?v=3']
assets=[]
...
p.write_text(text.rstrip()+'\n')
PY
grep -Fq '/ui/train-preview-parity-final-v1.css?v=3' "$INDEX"
grep -Fq 'No legacy desktop workspace is involved' "$DIST/ui/train-preview-parity-final-v1.css"
grep -Fq 'Future governed days such as' "$DIST/ui/train-preview-parity-final-v1.css"
grep -Fq 'Day 4 use the same premium desktop exercise-card hierarchy' "$DIST/ui/train-preview-parity-final-v1.css"
...
! grep -Fq '.lmf-desktop-workspace' "$DIST/ui/train-preview-parity-final-v1.css"
! grep -Fq '.lmf-desktop-flow-panel' "$DIST/ui/train-preview-parity-final-v1.css"
! grep -Fq '.lmf-desktop-context-panel' "$DIST/ui/train-preview-parity-final-v1.css"
grep -Fq "'/ui/train-preview-parity-final-v1.css'" "$SW"
echo 'LetMeFly final Train preview parity + no-legacy desktop ownership guard: PASS'
