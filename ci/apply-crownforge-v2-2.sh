...
def file_hashes(path: Path) -> dict[str, str]:
    return {str(file.relative_to(path)): sha(file) for file in sorted(p for p in path.rglob('*') if p.is_file())}
...
protected_before = {'blackCrown': tree_hash(black_crown), 'crownMaintenance': tree_hash(maintenance)}
cf_before = file_hashes(cf_root)
conditional_note = ('Conditional technique primer only: perform 2 x 10 when hinge timing or stiffness needs a ramp; '
...
count = meta_text.count("version: 'v2.1'")
if count != 1:
    raise SystemExit(f'Crownforge metadata expected one v2.1 version marker, found {count}')
metadata.write_text(meta_text.replace("version: 'v2.1'", "version: 'v2.2'", 1))
changed.append(str(metadata.relative_to(root)))

protected_after = {'blackCrown': tree_hash(black_crown), 'crownMaintenance': tree_hash(maintenance)}
...
actual_changed = sorted(f'src/programs/crownforge/{rel}' for rel in cf_before if cf_before[rel] != cf_after[rel])
...
marker = {
    'program': 'Crownforge', 'version': 'v2.2', 'baseVersion': 'v2.1',
    'revisionId': manifest['revisionId'], 'affectedWeeks': manifest['affectedWeeks'], 'affectedDay': 6,
    'changedFiles': actual_changed, 'protectedBefore': protected_before, 'protectedAfter': protected_after,
