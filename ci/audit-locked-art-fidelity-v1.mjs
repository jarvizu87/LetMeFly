...
assert(sw.includes('-locked-ui-v26'), 'service worker must refresh the previously cached UI assets')
for (const extension of ['js','css']) {
  const asset = `/ui/home-train-reference-v1.${extension}?v=${extension === 'js' ? 6 : 4}`
...
assert(index.includes('/ui/train-block-cards-v1.css?v=5') && sw.includes('/ui/train-block-cards-v1.css?v=5'), 'The block-card layout is installed and cached')
