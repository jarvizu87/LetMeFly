...
html = re.sub(r'/ui/progress-dashboard-v1\.js\?v=\d+', '/ui/progress-dashboard-v1.js?v=10', html)
html = re.sub(r'/ui/progress-dashboard-mount-guard\.js\?v=\d+', '/ui/progress-dashboard-mount-guard.js?v=10', html)
index.write_text(html)
PY

node --check "$BASE_JS"
node --check "$GUARD_OUT"
grep -Fq "const hash=location.hash||''; const progressHash=" "$BASE_JS"
grep -Fq 'if(hash&&' "$BASE_JS"
grep -Fq '!progressHash)return null' "$BASE_JS"
grep -Fq 'if(!onRoute)return null' "$BASE_JS"
grep -Fq 'function unmountProgressSurface()' "$GUARD_OUT"
grep -Fq '/ui/progress-dashboard-v1.js?v=10' "$INDEX"
grep -Fq '/ui/progress-dashboard-mount-guard.js?v=10' "$INDEX"

echo "LetMeFly Home/Progress route isolation: PASS (explicit non-Progress routes override stale Progress nav state)"
