...
node --test "$ROOT_DIR/ci/audit-athlete-insights.mjs"
mkdir -p "$DIST_DIR/ui/athlete-insights-v1"
for file in athlete-insights.mjs private-history.mjs athlete-insights-ui.mjs coach-rule-manifest.json athlete-insights.css; do
  cp "$ROOT_DIR/overlays/athlete-insights-v1/$file" "$DIST_DIR/ui/athlete-insights-v1/$file"
done
for file in athlete-insights.mjs private-history.mjs athlete-insights-ui.mjs; do node --check "$DIST_DIR/ui/athlete-insights-v1/$file"; done
...
assets += ['/ui/athlete-insights-v1/'+name for name in ['athlete-insights.mjs','private-history.mjs','athlete-insights-ui.mjs','coach-rule-manifest.json','athlete-insights.css']]
