(() => {
  ...
  // LetMeFly recording-led mobile regression fixes v3.
  ...
  function reconcilePreviewCards(shell, preview) {
    ...
    cards.forEach((card) => {
      card.querySelectorAll(':scope > .lmf-preview-plan-toggle').forEach((node) => node.remove())
      delete card.dataset.lmfPreviewCollapsed
      if (preview) card.dataset.lmfPreviewDay1Style = 'true'
      else delete card.dataset.lmfPreviewDay1Style
    })
  }
