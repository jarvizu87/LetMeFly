(() => {
  ...
  const cards = () => [...(page()?.querySelectorAll('.exercise-stack > .active-exercise, .exercise-stack > .preview-card') || [])]
  ...
  const current = () => cards().find(c => c.classList.contains('lmf-flow-active') || c.classList.contains('lmf-reference-preview-active')) || cards().find(c => !complete(c)) || cards()[0]
  const actionSources = new WeakMap()
  ...
  function render() {
    ...
    const preview = card?.classList.contains('preview-card') || false
    const previewRows = [...(card?.querySelectorAll('.prescription-block > .prescription-row') || [])]
      .map(node => [text(node.querySelector('strong')), text(node.querySelector('span'))].filter(Boolean).join(' • '))
    ...
    const signature = JSON.stringify([card?.dataset.exerciseId,row?.dataset.setId,name,prescription,load,loadUnit,dose,rpe,plate,section,preview,previewRows,cards().map(complete),rows(card).map(done),actions.map(a=>[a.key,text(a.node),disabled(a.node)])])
    if (body.dataset.lmfDesktopV2Signature === signature && body.querySelector('[data-lmf-desktop-v2-content]')) { bindActions(body, card, actions); return }
    body.innerHTML = preview ? `
      <div class="lmf-desktop-context-card" data-lmf-desktop-v2-content><small>Exercise Preview</small><strong style="font-size:20px">${esc(name)}</strong><p>${esc(section)}</p></div>
      <div class="lmf-desktop-context-card"><small>Programmed Work</small>${previewRows.map(value=>`<p>${esc(value)}</p>`).join('')}</div>
      <div class="lmf-desktop-context-card"><small>Exercise Tools</small><div class="lmf-desktop-v2-tools">${actions.map(a=>`<button type="button" class="lmf-desktop-v2-tool" data-lmf-desktop-v2-action="${a.key}"${disabled(a.node)?' disabled aria-disabled="true"':''}><span>${esc(text(a.node) || a.node.getAttribute('aria-label'))}</span><span aria-hidden="true">›</span></button>`).join('')}</div></div>
      <div class="lmf-desktop-context-card"><small>Workout Preview</small><p>Review the programmed movements here. Set logging becomes available when you start this workout.</p></div>` : card ? `
    ...
    bindActions(body, card, actions)
    const subtitle = panel.querySelector('.lmf-desktop-panel-head small')
    if (subtitle) subtitle.textContent = preview ? 'Programmed workout preview' : 'Live workout information'
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (forward) {
      ...
      const source = actionSources.get(forward)
      if (!disabled(forward) && !disabled(action) && source?.card === card && source?.node === action && card.contains(action)) action.click()
    }
    schedule()
  }, true)
