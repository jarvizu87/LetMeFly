...
import { CROWNFORGE, CROWN_MAINTENANCE, BLACK_CROWN, BLACK_CROWN_BLOCKS, getCalendarDay, getCrownforgeDay, getCrownforgeWeek, getCrownMaintenanceDay, getCrownMaintenanceWeek, getBlackCrownDay, getBlackCrownWeek, type PublicProgramKey, type ProgramDay, type ProgramExercise, type ProgramWeek } from './data/programs'
import {
...
type Route = 'home' | 'train' | 'program' | 'progress' | 'exercises' | 'coach' | 'profile' | 'calendar' | 'settings' | 'more'
...
async function boot(): Promise<void> {
  try {
    ...
    root.innerHTML = `<main><div class="card empty-state"><div class="page-kicker">Private vault unavailable</div><h1>LetMeFly could not open local storage</h1><p class="muted">Your athlete data was not deleted. The app could not open the private local vault on this attempt.</p><div class="callout warning">${esc(errorMessage(error))}</div><a class="btn primary" href="/" style="margin-top:12px">RETRY LETMEFLY</a></div></main>`
  }
}

function getProgramDay(program: PublicProgramKey, week: number, day: number): ProgramDay | null {
...
function topbar(): string {
  const routeNames: Record<Route, string> = {
    home: 'Command', train: 'Train', program: 'Program', progress: 'Progress', exercises: 'Exercises', coach: 'Coach', profile: 'Profile', calendar: 'Calendar', settings: 'Settings', more: 'More',
  }
  return `
    <header class="topbar">
      ...
      <div class="topbar-context"><strong>${routeNames[state.route]}</strong><span>${state.selectedProgram === 'black-crown' ? 'BC' : state.selectedProgram === 'crown-maintenance' ? 'CM' : 'CF'} • W${state.selectedWeek} • D${state.selectedDay}</span></div>
  ...
  const moreActive = ['exercises', 'coach', 'profile', 'calendar', 'settings', 'more'].includes(state.route)
  return `<nav class="navbar" aria-label="Primary navigation">${items.map(([route, icon, label]) => {
    const active = route === 'more' ? moreActive : state.route === route
    return `<a class="nav-item ${active ? 'active' : ''}" href="#/${route}"><span>${icon}</span>${label}</a>`
  }).join('')}</nav>`
  ...
  return `
    <section class="command-hero">
      ...
      <div class="hero-content">
        <div class="hero-brandline"><span>${esc(homeProgramName.toUpperCase())}</span><em>${homeProgram === 'black-crown' ? 'v2.0' : 'v2.1'}</em></div>
    ...
    <div class="dashboard-grid">
      ...
      <section class="card status-card">
        ...
        <div class="performance-list">
          <div><span>Program</span><strong>${esc(homeProgramName)}</strong></div>
      ...
      <section class="card milestone-card">
        ...
        <div><div class="page-kicker">Next Milestone</div><h2>${homeProgram === 'black-crown' ? `Black Crown Block ${Math.ceil((today?.week ?? state.selectedWeek) / 6)} completion` : 'Verified testing • Weeks 13–14'}</h2><p class="muted">${homeProgram === 'black-crown' ? 'Complete the current governed six-week block before advancing.' : 'Black Crown entry uses the governed Crownforge testing + Crown Maintenance handoff.'}</p></div>
  ...
  const bodyweights = (await getAll<LocalDomainRecord>('bodyweightEntries')).filter((row) => row.athlete_id === state.athlete?.id).sort((a,b) => String(b.recorded_at ?? '').localeCompare(String(a.recorded_at ?? '')))
  const prs = (await getAll<LocalDomainRecord>('personalRecords')).filter((row) => row.athlete_id === state.athlete?.id)
  const lifts = [
  ...
  const strengthCards = lifts.slice(0,4).map(([key,name]) => {
    ...
    return `<div class="strength-metric ${value ? '' : 'empty'}"><span>${esc(name)}</span><strong>${value ? esc(String(value)) : '—'}<small>${value ? ` ${esc(String(row?.tm_unit ?? 'lb'))}` : ''}</small></strong><i style="--strength:${value ? Math.max(12, Math.min(100, value / 4.5)) : 0}%"></i></div>`
  }).join('')
  target.innerHTML = `<div class="progress-score-grid command-metrics">
    ...
    <div class="section-title"><div><div class="page-kicker">History</div><h2>Recent Workouts</h2></div></div><div class="history-list">${workouts.length ? workouts.map((w) => `<div class="history-card"><div class="history-icon">✓</div><div><span>${esc(String(w.started_at ?? w.scheduled_for ?? '')).slice(0,10)}</span><strong>${esc(String(w.workout_name ?? 'Workout'))}</strong><small>${esc(String(w.program_key ?? ''))} • W${esc(String(w.week_number ?? ''))} • ${esc(String(w.day_key ?? ''))}</small></div><em>${esc(String(w.status ?? ''))}</em></div>`).join('') : '<div class="card empty empty-state"><div class="page-kicker">History starts here</div><h3>No workouts logged yet.</h3><p class="muted">Your first started session will appear here without changing the programmed workout.</p><a class="btn ghost" href="#/train">OPEN TRAIN</a></div>'}</div>`
  bindProgressEvents()
  } catch (error) {
    console.warn('Progress history unavailable', error)
    target.innerHTML = `<div class="card empty empty-state"><div class="page-kicker">Progress unavailable</div><h2>Training history could not be read.</h2><p class="muted">Your workout data remains in the private local vault. You can keep training locally and retry this view.</p><div class="callout warning">${esc(errorMessage(error))}</div><button class="btn ghost" data-action="retry-progress" style="margin-top:12px">RETRY PROGRESS</button></div>`
    target.querySelector('[data-action="retry-progress"]')?.addEventListener('click', () => void renderProgressAsync())
  }
}
function exercisesPage(): string {
  const programNames = new Set<string>()
  for (const week of [...CROWNFORGE.weekData, ...CROWN_MAINTENANCE.weekData, ...BLACK_CROWN.weekData]) {
  ...
  return `<div class="coach-shell">
    ...
    <div class="coach-layout">
      <section class="coach-context card"><div class="page-kicker">Current Context</div><h2>${esc(day?.title ?? selectedProgramName())}</h2><div class="context-grid"><div><span>Program</span><strong>${esc(selectedProgramName())}</strong></div><div><span>Position</span><strong>W${state.selectedWeek} • D${state.selectedDay}</strong></div><div><span>Priority</span><strong>Program rules</strong></div><div><span>Session</span><strong>${state.workout ? `${completionStats(state.workout).percent}% logged` : 'Not started'}</strong></div></div><p>${esc(day?.readinessRule ?? 'Preserve the program and train with clean technique.')}</p></section>
      <section class="coach-chat card"><div id="coach-answer" class="coach-message"><div class="coach-message-mark">♛</div><div><span>LETMEFLY COACH</span><h2>${esc(day?.title ?? selectedProgramName())}</h2><p>${esc(day?.readinessRule ?? 'Preserve the program and train with clean technique.')}</p></div></div><div class="quick-prompts"><button data-coach-prompt="What are we doing today?">WHAT ARE WE DOING TODAY?</button><button data-coach-prompt="Can I increase the weight?">CAN I INCREASE THE WEIGHT?</button><button data-coach-prompt="Why am I doing this?">WHY IS THIS HERE?</button><button data-coach-prompt="Give me a substitute.">GIVE ME A SUBSTITUTE</button><button data-coach-prompt="What should I focus on?">WHAT SHOULD I FOCUS ON?</button><button data-coach-prompt="What did I do last time?">WHAT DID I DO LAST TIME?</button></div><div class="coach-composer"><textarea id="coach-question" rows="2" placeholder="Ask your coach anything…"></textarea><button class="btn primary" data-action="ask-coach">ASK</button></div></section>
    </div>
  </div>`
  ...
  return `<div class="page-head cinematic-head"><div class="page-kicker">Everything else</div><h1>MORE</h1><p class="muted">Every LetMeFly screen and athlete utility in one command menu.</p></div><div class="more-grid command-menu-grid">
    <a class="more-card" href="#/home"><span>⌂</span><div><strong>Home</strong><small>Command dashboard.</small></div><i>›</i></a>
    <a class="more-card" href="#/train"><span>⚔</span><div><strong>Train</strong><small>Immersive workout mode.</small></div><i>›</i></a>
    ...
    <a class="more-card" href="#/progress"><span>↗</span><div><strong>Progress</strong><small>Strength and training analytics.</small></div><i>›</i></a>
    ...
    <a class="more-card" href="#/settings"><span>⚙</span><div><strong>Settings</strong><small>App and equipment settings.</small></div><i>›</i></a>
    <a class="more-card" href="#/profile"><span>⇩</span><div><strong>Data & Backup</strong><small>Export, restore, migrate, sync.</small></div><i>›</i></a>
    <a class="more-card" href="#/coach"><span>?</span><div><strong>Help & Support</strong><small>Coaching and app guidance.</small></div><i>›</i></a>
...
function calendarPage(): string {
  const days = CROWNFORGE.weekData.flatMap((week) => week.days.map((day) => ({ ...day, week: week.week })))
  ...
  return `<div class="calendar-shell"><div class="page-head cinematic-head"><div class="page-kicker">Plan & stay on track</div><h1>CALENDAR</h1><p class="muted">${esc(monthName)} • governed Crownforge schedule</p></div><section class="card calendar-month-card"><div class="calendar-grid">${['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((x) => `<div class="calendar-head">${x}</div>`).join('')}${cells.join('')}</div></section><div class="calendar-list">${days.slice(0,12).map((day) => `<div class="calendar-event ${day.restDay ? 'rest' : ''}"><i></i><div><span>${day.date ? esc(day.date) : 'Program'}</span><strong>${esc(day.title)}</strong><small>${esc(day.role)}</small></div><em>W${day.week} D${day.day}</em></div>`).join('')}</div></div>`
}
...
function openExerciseDemo(programName: string): void {
  if (!navigator.onLine) {
    showModal(`<div class="page-kicker">Exercise demo offline</div><h2>${esc(programName)}</h2><div class="callout warning">Video and search links need an internet connection. Workout logging, prescriptions, Exercise Info, and approved substitution guidance remain available offline.</div><button class="btn primary" data-close-modal style="margin-top:12px">Close</button>`)
    return
  }
...
async function hydrateHomeReadiness(): Promise<void> {
  ...
  if (note) {
    ...
    note.textContent = `${color.toUpperCase()} readiness${recorded ? ` • ${recorded}` : ''}. Readiness stays inside governed auto-regulation limits and never rewrites the selected governed program.`
  }
}
...
function readinessInputFromForm(): { sleepQuality: number; energy: number; soreness: number; stress: number; notes: string | null } | null {
...
async function startSelectedWorkout(): Promise<void> {
  ...
  state.workout = await startWorkout(state.athlete.id, state.programInstance?.id ?? null, state.selectedProgram, state.selectedWeek, day, readiness.id)
  state.cloud.syncSoon()
  showToast(`Readiness ${readinessColor(input).toUpperCase()} • workout saved locally`)
  render()
}
